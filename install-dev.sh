...
if [ "$1" = "--build" ]; then
  BUILD_MODE=true
  echo -e "${BLUE}🚀 n8n-nodes-kie - Build & Publish Mode${NC}"
else
  echo -e "${GREEN}🚀 n8n-nodes-kie Development Installer${NC}"
fi
echo ""
...
# Increase Node memory limit to prevent V8 crashes on Node v22/WSL2
export NODE_OPTIONS="--max-old-space-size=4096"

pnpm run build
if [ $? -ne 0 ]; then
  echo -e "${RED}Error: Build failed${NC}"
  echo -e "${YELLOW}Tip: Try with Node v20 LTS if issues persist (Node v22 can have WSL2 issues)${NC}"
  exit 1
fi
...
if [ "$BUILD_MODE" = true ]; then
  ...
  if [ $? -eq 0 ]; then
    echo ""
    echo -e "${GREEN}✅ Successfully published n8n-nodes-kie@${NEW_VERSION} to npm!${NC}"
    echo ""
    echo -e "${YELLOW}Next steps:${NC}"
    echo "- Commit the version bump: git add package.json pnpm-lock.yaml && git commit -m 'chore: bump version to ${NEW_VERSION}'"
...
if [ $? -eq 0 ]; then
  ...
  if [ -d "$N8N_PATH" ]; then
    cd "$N8N_PATH" && npm link n8n-nodes-kie
    cd - > /dev/null
...
echo ""
echo -e "${YELLOW}To use the Kie.ai node:${NC}"
echo "1. Stop n8n if it's running (Ctrl+C)"
echo "2. Set environment variable (REQUIRED):"
...
echo "- Verify pnpm is installed: pnpm --version"
